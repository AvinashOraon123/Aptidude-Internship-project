...
def main():
    ...
    print("\n--- Stage 5: Writing Results back to DB for Power BI ---")
    conn = sqlite3.connect(db_path)

    q_diff_table.to_sql('recalibrated_questions', conn, if_exists='replace', index=False)
    student_labeled.to_sql('student_segments', conn, if_exists='replace', index=False)
    recs_df.to_sql('recommendations', conn, if_exists='replace', index=False)

    latest_snapshots = train_df.sort_values('snapshot_ts').groupby('user_id').tail(1)
    X_current = latest_snapshots[dm.FEATURE_COLS]
    risk_probs = dropoff_results['xgb_model'].predict_proba(X_current)[:, 1]
    risk_df = pd.DataFrame({
        'user_id': latest_snapshots['user_id'],
        'risk_score': risk_probs
    })
    risk_df.to_sql('dropoff_risk', conn, if_exists='replace', index=False)

    conn.close()
    print(f"All results written back to {db_path}. Power BI can now connect to these tables.")

    print("\n--- Pipeline Complete ---")
    print(f"All results saved to {output_dir} and {db_path}")
